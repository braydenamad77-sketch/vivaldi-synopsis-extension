...
const BROWSER_IDLE_MS = Number(process.env.GOODREADS_HELPER_BROWSER_IDLE_MS) || 1000 * 60 * 3;
...
function buildUrlCandidates(input = {}) {
  ...
  for (const id of goodreadsIds) {
    urls.push(`https://www.goodreads.com/book/show/${encodeURIComponent(id)}`);
  }

  for (const isbn of [...isbn13, ...isbn10]) {
    urls.push(`https://www.goodreads.com/book/isbn/${encodeURIComponent(isbn)}`);
  }

  if (title) {
    urls.push(`https://www.goodreads.com/book/title?id=${encodeURIComponent(title)}`);
  }
...
export async function captureGoodreadsDescriptionScreenshots(input = {}) {
