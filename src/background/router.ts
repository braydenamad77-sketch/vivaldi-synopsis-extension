import { DEFAULT_SETTINGS, LLM_SOURCE_TEXT_MAX_WORDS } from "../config/constants";
import { buildCacheKey, normalizeQuery } from "../core/normalize";
import {
...
} from "../core/disambiguate";
import { getCache, setCache } from "../core/cache";
import { mirrorCacheEntryToCompanionApp } from "../core/companion-app";
import { appendDebugEvent, getDebugState } from "../debug/store";
import { pickRandomGoodreadsTestSeed } from "../debug/goodreads-test-seeds";
import type { GoodreadsTestDebugEvent, LookupDebugEvent } from "../debug/types";
import { sanitizeSynopsis, safeTemplate, trimToWordLimit } from "../core/spoiler-guard";
import { searchOpenLibrary, fetchOpenLibraryDetails } from "../providers/openlibrary";
import { searchTmdb, fetchTmdbDetails } from "../providers/tmdb";
import { fetchTvmazeArtwork } from "../providers/tvmaze";
import { fetchWikipediaSummaryByTitle, searchWikipediaCandidates } from "../providers/wikipedia";
import { fetchGoodreadsFallback } from "../providers/goodreads";
import { rewriteSynopsisWithOpenRouter } from "../llm/openrouter";
import { normalizeTitleForCompare } from "../core/normalize";
import type {
  AnyRecord,
  Candidate,
  ExtensionSettings,
...
type WikipediaFallbackDecision =
  | { status: "resolved"; candidate: Candidate; gap: number }
  | { status: "rejected"; reason: string; candidate?: Candidate; gap?: number };

const pendingAmbiguities = new Map<string, PendingAmbiguity>();
...
export async function getSettings() {
  ...
  return mergeSettings(payload.settings as Partial<ExtensionSettings> | undefined);
}

function buildAttribution(source: string | undefined, _llmUsed: boolean) {
  return source || "Unknown";
}

function shouldBypassCachedResult(cached: LookupResult | undefined, settings: ExtensionSettings) {
...
function mediaTypeLabel(mediaType: string | undefined) {
...
function buildGenreLabel(details: LookupDetails) {
  const genres = Array.isArray(details.genres) ? details.genres.filter(Boolean) : [];
  if (!genres.length) return undefined;
  return genres.slice(0, 2).join(" / ");
}

function buildGenreSource(details: LookupDetails, genreLabel: string | undefined) {
...
function buildTags(details: LookupDetails) {
...
function buildProviderHealthNote(providerHealth: ProviderHealth) {
...
function buildNotFoundResponse(normalized: NormalizedQuery, providerHealth: ProviderHealth): LookupResponse {
...
function buildLookupQuery(normalized: NormalizedQuery) {
...
async function appendLookupDebugEvent(
  event: Omit<LookupDebugEvent, "id" | "createdAt" | "kind" | "title"> & { title?: string },
) {
  const debugState = await getDebugState();
  if (!debugState.enabled) return;

  const nextEvent: LookupDebugEvent = {
  ...
  await appendDebugEvent(nextEvent);
}

export function chooseWikipediaFallbackCandidate(
  ranked: Candidate[],
  normalized: NormalizedQuery,
): WikipediaFallbackDecision {
  if (!ranked?.length) {
    return {
      status: "rejected",
  ...
  const gap = second ? Number((topScore - Number(second.score || 0)).toFixed(6)) : topScore;
...
function appendSourceAttribution(current: string | undefined, next: string | undefined) {
...
function createPendingAmbiguity({ normalized, settings, candidates, cacheKey, note }: PendingAmbiguity) {
...
function withChooserMeta(
  result: LookupResult,
  { lookupQuery, canChooseAnother, reselectRequestId }: { lookupQuery: string; canChooseAnother: boolean; reselectRequestId?: string },
): LookupResult {
...
function toCacheableResult(result: LookupResult): LookupResult {
  const cacheable = { ...result };
  delete cacheable.reselectRequestId;
  return cacheable;
}

function fromCachedResult(cached: LookupResult, settings: ExtensionSettings): LookupResult {
...
async function lookupCandidates(normalized: NormalizedQuery, settings: ExtensionSettings) {
  const jobs: Array<{ provider: keyof ProviderHealth; promise: Promise<Candidate[]> }> = [];
...
async function lookupWikipediaCandidates(
  normalized: NormalizedQuery,
  settings: ExtensionSettings,
  providerHealth: ProviderHealth,
  limit = WIKIPEDIA_FALLBACK_LIMIT,
): Promise<Candidate[]> {
...
function mergeCandidates(primaryCandidates: Candidate[], wikiCandidates: Candidate[]): Candidate[] {
  const seen = new Set();
...
async function applySynopsisPipeline(details: LookupDetails, settings: ExtensionSettings): Promise<LookupDetails> {
...
function toResult(details: LookupDetails, settings: ExtensionSettings, fromCache = false): LookupResult {
...
async function buildWideSearchAmbiguity({
  normalized,
  settings,
  providerHealth,
  primaryCandidates,
...
export async function lookupSynopsis(request: AnyRecord): Promise<LookupResponse> {
  ...
  const cached = await getCache<LookupResult>(cacheKey);
  ...
  const details = await hydrateCandidate(decision.candidate as Candidate, settings, normalized);
  ...
  await appendLookupDebugEvent({
    ...
    detail: {
      resolvedCandidate: debugCandidateLabel(decision.candidate as Candidate),
      rankedCandidates: ranked.slice(0, 5).map(debugCandidateLabel),
...
export async function requestAlternatives(request: AnyRecord): Promise<LookupResponse> {
...
export async function resolveAmbiguity(request: AnyRecord): Promise<LookupResponse> {
  ...
  const selected = pending.candidates.find((candidate: Candidate) => candidate.id === request.selectedCandidateId);
  if (!selected) {
    return {
      status: "error",
...
export async function runGoodreadsVisualDebugTest() {
  const settings = await getSettings();
  const seed: any = pickRandomGoodreadsTestSeed();
  const input = {
    title: seed.title,
    author: seed.author,
    year: seed.year,
  ...
  const fallback: any = await fetchGoodreadsFallback(input, settings, {
    skipCache: true,
    includeDebugAssets: true,
  });

  const eventBase: Omit<GoodreadsTestDebugEvent, "error" | "synopsisLlmOutput" | "synopsisRequest" | "finalSynopsis" | "finalGenres"> = {
    id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    createdAt: new Date().toISOString(),
    kind: "goodreads_test",
    title: seed.title,
  ...
  try {
    const rewritten: any = await rewriteSynopsisWithOpenRouter(
      {
        title: seed.title,
        mediaType: "book",
        year: seed.year,
    ...
    await appendDebugEvent({
      ...eventBase,
      status: "error",
      error: error instanceof Error ? error.message : "Synopsis rewrite failed.",
      synopsisRequest: {},
      synopsisLlmOutput: "",
      finalSynopsis: "",
      finalGenres: [],
    ...
    return {
      status: "error",
      title: seed.title,
      message: error instanceof Error ? error.message : "Synopsis rewrite failed.",
    };
  }
}
