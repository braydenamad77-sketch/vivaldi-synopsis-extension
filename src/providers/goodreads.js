...
export function normalizeGoodreadsHelperUrl(value) {
...
function buildLookupPayload(meta = {}, settings = {}) {
  return {
...
function goodreadsCacheKey(payload) {
  return `cache:goodreads-visual:v1:${hashString(JSON.stringify(payload))}`;
}

async function postHelper(helperUrl, payload) {
  ...
  try {
    ...
    return {
      status: "helper_unavailable",
      debug: {
        reason: error?.message || "Could not reach local Goodreads helper.",
      },
    };
  } finally {
...
export async function fetchGoodreadsFallback(meta = {}, settings = {}, options = {}) {
  ...
  const cached = !options.skipCache ? await getCache(cacheKey) : undefined;
