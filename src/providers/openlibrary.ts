...
function first<T>(value: T[] | T | undefined, fallback = "") {
...
function splitIsbns(values: string[] | string | undefined) {
...
function normalizeGoodreadsIds(values: string[] | string | undefined) {
  const list = Array.isArray(values) ? values : values ? [values] : [];
  const ids: string[] = [];
  const seen = new Set();
  for (const value of list) {
...
export async function searchOpenLibrary(normalizedQuery: NormalizedQuery): Promise<Candidate[]> {
  ...
  const data = (await fetchJson(url, OPENLIBRARY_SEARCH_TIMEOUT_MS)) as AnyRecord;
  const docs = data.docs || [];

  return docs.map((doc: AnyRecord) => {
...
function extractDescription(payload: AnyRecord | null) {
...
function extractBookGenres(work: AnyRecord | null) {
...
function extractEditionDescription(editions: AnyRecord | null) {
...
function extractEditionGenres(editions: AnyRecord | null) {
  const entries = Array.isArray(editions?.entries) ? editions.entries : [];
  const merged: string[] = [];
...
function extractEditionGoodreadsIds(editions: AnyRecord | null) {
  const entries = Array.isArray(editions?.entries) ? editions.entries : [];
  const merged: string[] = [];
...
function extractEditionIsbns(editions: AnyRecord | null) {
  const entries = Array.isArray(editions?.entries) ? editions.entries : [];
  const all: string[] = [];
...
function dedupeMerge(primary: string[] = [], secondary: string[] = [], limit = 6) {
  const merged: string[] = [];
...
export async function fetchOpenLibraryDetails(candidate: Candidate) {
  const workKey = candidate.id?.startsWith("/works/") ? candidate.id : null;
  let description = "";
  let genres: string[] = [];
  let editionDescription = "";
  let editionGenres: string[] = [];
  let editionGoodreadsIds: string[] = [];
  let editionIsbns: { isbn10: string[]; isbn13: string[] } = { isbn10: [], isbn13: [] };
