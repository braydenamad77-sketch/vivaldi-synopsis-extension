...
import { AMBIGUITY_CANDIDATE_MAX, chooseCandidate, collapseBookCandidates, rankCandidates } from "../src/core/disambiguate.js";
...
test("rankCandidates clamps book recency boost between 1900 and 2025", () => {
  ...
  assert.equal(scoreById.get("below-floor"), scoreById.get("at-floor"));
  assert.equal(scoreById.get("at-ceiling"), scoreById.get("above-ceiling"));
  assert.equal(scoreById.get("at-ceiling") > scoreById.get("at-floor"), true);
});
...
test("rankCandidates handles missing or invalid book years safely", () => {
  const ranked = rankCandidates(
    [
      ...
      { id: "invalid-year", provider: "openlibrary", title: "Atlas", mediaType: "book", year: "unknown" },
